...
034 & smart\_collections & System smart collections with filters \\
035 & reading\_goals & Reading goals and streaks tracking \\
036 & fix\_partitioned\_rls & RLS policies for partitioned tables \\
037 & hide\_analytics\_views & Hide materialized views from API \\
038 & series\_grouping & Book series management \\
039 & archive\_system & Archive/trash functionality \\
040 & filter\_presets & Saved filter configurations \\
041 & collection\_templates & Pre-configured collection structures \\
042 & book\_recommendations & AI-powered book recommendations \\
043 & remove\_unused\_partitioning & Remove unused partition infrastructure \\
\bottomrule
\end{longtable}
...
\begin{itemize}
    ...
    \item \texttt{book\_series} - Multi-volume book series
    \item \texttt{reading\_goals} - Reading objectives and tracking
    \item \texttt{collection\_templates} - Pre-configured collection structures
    \item \texttt{filter\_presets} - Saved filter combinations
...
\begin{itemize}
    ...
    \item \texttt{get\_smart\_collection\_books(UUID)} - Get books in smart collection
    \item \texttt{update\_reading\_goals()} - Update goals on activity
    \item \texttt{create\_collections\_from\_template(UUID, UUID)} - Apply template
    \item \texttt{get\_unified\_recommendations(UUID, INT)} - AI recommendations
    \item \texttt{get\_series\_books(UUID)} - Books in a series
    \item \texttt{archive\_book(UUID)} - Archive a book
    \item \texttt{get\_archived\_books(UUID)} - List archived books
    \item \texttt{get\_popular\_templates(INT)} - Most used templates
\end{itemize}
