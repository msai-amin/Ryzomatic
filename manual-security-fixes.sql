-- Manual Security Fixes for Supabase Dashboard SQL Editor
...
-- Step 2: Drop functions (they will be recreated with security settings)
...
-- Step 3: Recreate functions with security settings
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COALESCE(SUM(pomodoro_sessions_count * 25), 0) FROM public.user_books WHERE user_id = user_uuid) as total_reading_time,
...
-- Note: Only recreate triggers for functions we've defined above
