...
const CapacitorTest = () => {
  ...
  useEffect(() => {
    const checkPlatform = async () => {
      try {
        ...
        addTestResult(`Running in Capacitor: ${isCapacitor}`)
        ...
        if (typeof (window as any).Capacitor !== 'undefined') {
          ...
          addTestResult(`Capacitor available: ${Capacitor.getPlatform()}`)
          addTestResult(`Native platform: ${Capacitor.isNativePlatform()}`)
        } else {
          addTestResult('Capacitor not available globally')
        }
      } catch (err) {
  ...
  const testCapacitorModules = async () => {
    ...
    try {
      ...
      // Try to access Capacitor from global scope
      const Capacitor = (window as any).Capacitor
      if (!Capacitor) {
      ...
      // Try to dynamically import filesystem
      try {
        ...
        addTestResult(`❌ Filesystem import failed: ${importError}`)
        throw importError
      }
    } catch (err) {
  ...
  const testLocalStorage = async () => {
    setIsLoading(true)
    setError(null)
    addTestResult('Testing local storage...')

    try {
      // Test localStorage
      const testKey = 'smart-reader-test'
      const testValue = 'Hello from Smart Reader!'
      
      localStorage.setItem(testKey, testValue)
      const retrieved = localStorage.getItem(testKey)
      
      if (retrieved === testValue) {
        addTestResult('✅ localStorage works correctly')
      } else {
        throw new Error('localStorage test failed')
      }
      
      localStorage.removeItem(testKey)
      addTestResult('Cleaned up localStorage test')
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'localStorage error'
      setError(errorMessage)
      addTestResult(`❌ localStorage error: ${errorMessage}`)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>Capacitor Environment: {isNative ? '✅ Yes' : '❌ No'}</div>
      ...
      <div style={{
        ...
        <ul style={{
          ...
          <li>This demo tests basic functionality on your iPad Pro M4</li>
          <li>Click "Test Capacitor" to verify file system access (if available)</li>
